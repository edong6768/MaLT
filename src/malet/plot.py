                ...
                for ci, col_v in enumerate(col_vs):
                    for ri, row_v in enumerate(row_vs[::-1]):
                        ...
                        for mlvs, st in zip(mlines, styles):
                            ...
                            init_prgs += 1
                            progress.update(task, description=f"[green]Drawing plot... [{init_prgs}/{total_prgs}]", advance=1)

                        # add current animation field value as text on first axes
                        if pani and ri+ci==0:
                            ani_artists[-1].append(
                                ax.text(0.0, 1.01, f'{pani}={aniv}', fontsize=pcfg['font_size'], ha='left', va='bottom', transform=ax.transAxes)
                            )
...
def run(argv, preprcs_df):
    ...
    if plot_config['animation_field']:
        with Progress() as progress:
            ...
            ani.save(os.path.join(save_dir, f'{save_name}.gif'), writer=gifwriter, 
                        progress_callback=lambda i, n: progress.update(task, description=f'[green]Saving animation frames [{i+1}/{len(ani_artists)}]', advance=1))
    else:
